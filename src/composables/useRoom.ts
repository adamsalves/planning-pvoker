...
import type { JoinableRole, DeckType, RoomConfig, PlayerTag } from '@/types'
...
export function useRoom() {
  ...
  async function createRoom(
    ...
    userStore.setPlayer(playerName, playerId, 'admin', tag)
    ...
    try {
      await socketJoin(roomId, { id: playerId, name: playerName, role: 'admin', tag }, config)
  ...
  async function joinRoom(
    playerName: string,
    roomCode: string,
    role: JoinableRole = 'member',
    tag?: PlayerTag,
  ) {
    const playerId = uuidv4()

    // Save player local
    userStore.setPlayer(playerName, playerId, role, tag)

    // Await before navigating (same reason as createRoom: store the token first).
    try {
      await socketJoin(roomCode, { id: playerId, name: playerName, role, tag })
